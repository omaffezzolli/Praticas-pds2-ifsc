...
public class lojas_parceirasDAO {
	...
	public boolean excluir(lojas_parceiras lp) {
		Conexao c = Conexao.getInstancia();
		Connection con = c.conectar();

		String query = "DELETE FROM lojas_parceiras WHERE id_cnpj = ?";
	...
	public boolean atualizar(lojas_parceiras lp) {
		Conexao c = Conexao.getInstancia();
		Connection con = c.conectar();

		String query = "UPDATE lojas_parceiras SET " + "nome_loja = ? Where nome_loja";
